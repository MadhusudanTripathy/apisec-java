...
public class EndpointClassifier {
  ...
  public record EndpointContext(String type, Set<String> types, boolean hasObjectIdentifier, boolean hasPaginationSignal,
  ...
  public static EndpointContext classify(RequestModel req) {
    ...
    String endpointType = detectEndpointType(req, body, path);
    ...
    return new EndpointContext(endpointType, types, objectId, pagination, otp, urlInput, cookie, tenant, payment, sensitiveAction);
  }

  private static String detectEndpointType(RequestModel req, String body, String path) {
    String contentTypes = lower(String.join(" ", headerValues(req, "Content-Type")));
    String soapAction = lower(String.join(" ", headerValues(req, "SOAPAction")));

    if (path.contains("graphql")
        || contentTypes.contains("application/graphql")
        || looksLikeGraphQlJson(body)
        || looksLikeGraphQlDocument(body)) {
      return "GRAPHQL";
    }

    if (contentTypes.contains("application/soap+xml")
        || contentTypes.contains("text/xml")
        || !soapAction.isBlank()
        || body.contains("schemas.xmlsoap.org/soap/envelope")
  ...
  private static boolean looksLikeGraphQlJson(String body) {
    return (body.contains("\"query\"") || body.contains("\"mutation\"") || body.contains("\"variables\"") || body.contains("\"operationname\""))
        && (body.contains("{") || body.contains("query ") || body.contains("mutation "));
  }

  private static boolean looksLikeGraphQlDocument(String body) {
  ...
  private static List<String> headerValues(RequestModel req, String name) {
    List<String> values = new ArrayList<>();
    req.headers.forEach((key, value) -> {
      if (key != null && key.equalsIgnoreCase(name) && value != null) values.addAll(value);
    });
    return values;
  }
